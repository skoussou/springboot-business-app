...
** then in the `namespace` where the Jenkins pipeline will build/deliver the generated *Service Image* and register it within the *ImageStream* (eg `dev-pam-pipeline`) that namespace will need to contain `system:image-puller` for *ServiceAccount* of the namespaces the image will be rolled out to:

	oc policy add-role-to-user system:image-puller system:serviceaccount:${STAGE/PROD_NAMESPACE}:default -n -n ${NAMESPACE_WHERE_SERVICE_WILL_BE_DEPLOYED}
...
** Deployment Configs need to be created with the following command (modifying for each namespace *NAMESPACE_TO_DEPLOY_TO* and *NAMESPACE_TO_BUILD_AT*

      oc create -f ocp-resources/business-application-service-deployment.yml -n stage-pam-pipeline
      oc create -f src/main/fabric8/service.yml -n stage-pam-pipeline
      oc expose business-application-service -n stage-pam-pipeline
      oc create -f ocp-resources/business-application-service-deployment.yml -n prod-pam-pipeline
      oc create -f src/main/fabric8/service.yml -n prod-pam-pipeline
      oc expose business-application-service -n prod-pam-pipeline

** Create databases as needed in these namespaces
